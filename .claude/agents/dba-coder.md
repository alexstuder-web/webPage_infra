...
<!-- LESSONS:START -->
- **2026-05-25 — `ALTER DEFAULT PRIVILEGES FOR ROLE postgres` is a no-op when supabase_admin creates all objects; use `FOR ROLE supabase_admin` (or omit `FOR ROLE` to default to the running role).** Why: DEFAULT PRIVILEGES only fire for objects created BY the specified role; since `postgres` is not a superuser and never creates application tables, the grant silently never applies to any real object. The rapt section in the same baseline correctly omits `FOR ROLE` (runs as supabase_admin). Apply the same pattern in the aibrewgenius section. (seen in `webPage_infra/db_scripts/baseline_schema.sql` lines 707-712)
- **2026-05-25 — Never use implicit row-found detection in PL/pgSQL; always use `FOUND` or an explicit counter.** Why: (a) `IF NOT FOUND` after `FOR … IN query LOOP` is wrong — `FOUND` is true as soon as the cursor yields one row, regardless of whether the loop body did meaningful work; use an explicit integer counter instead. (b) `IF <record_var> IS NOT NULL` after `SELECT INTO <record_var>` is fragile — it only works correctly when the selected column is a non-nullable PK; if the column is nullable the check silently gives a false negative. Use `IF FOUND THEN` immediately after any `SELECT INTO` to test whether a row was returned. (seen in `brew_assistent-new/db_scripts/migrations/006_retire_aibrewgenius_rapt.sql` — cursor loop; `RAPT_Brewing_Dashboard-new/db_scripts/005_rapt_telemetry_owner.sql` — `derive_brew_sessions_for` record-var check)
