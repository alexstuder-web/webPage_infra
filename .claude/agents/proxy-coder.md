...
<!-- LESSONS:START -->
- **2026-05-27 — Use `envStr(name, def)` / `envNum(name, def)` for every optional tuning var that docker-compose may inject as `${VAR:-}`.** Why: `process.env.X ?? default` and `Number(process.env.X ?? default)` both silently treat an empty string as a value — `??` does not trigger on `""`, and `Number("")===0` — so compose's empty-string injection produces `fetch("")`, `setInterval(fn, 0)`, or zero-ms timeouts instead of the intended defaults. All new optional vars MUST go through the helpers; only non-tuning vars (mandatory secrets, PROXY_ROLE, PROXY_ENV path) keep raw `process.env`. (seen in `db-sync.js`, `server.js`)
