...
info "  docker exec -e PGPASSWORD=... ${DB_CONTAINER} psql --no-psqlrc -U ${DB_USER} -d ${DB_NAME} \\"
...
info "  1. proxy_sync-Passwort prüfen: grep PROXY_SYNC_PASSWORD ~/webPage_infra/.env"
info "     + docker logs api-proxy (Verbindungsfehler prüfen)"
info "  2. App-Container deployen (Watchtower / manuell)"
info "  3. Post-Deploy-Gates (via bootstrap.sh-Ausgabe nach DB-Baseline oder manuell):"
info "     Gate A — Hypertable-Restore-Test (count(*) FROM rapt.telemetry_controllers > 0)"
info "     Gate B — SSO cross-subdomain (sb-session Cookie Domain=.alexstuder.cloud)"
info "     Gate C — Tenant-Isolation (zweiter User sieht keine Daten des ersten)"
